...
const DEFAULT_WHATSAPP = 'https://wa.me/972500000000'
const WhatsAppFloating = () => {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/about`).then(res => {
      ...
      if (res.data?.phone) setPhone(res.data.phone.replace(/\D/g, ''))
    }).catch(() => {})
  }, [])

  const telHref = phone ? `tel:+972${phone.replace(/^0/, '')}` : null
  ...
  return (
    <div className="floating-contact-buttons">
      <a
        ...
        aria-label="לכל שאלה ניתן לפנות"
        title="לכל שאלה ניתן לפנות"
      >
      ...
      {telHref && (
        <a
          ...
          title="התקשר"
        >
          <span className="phone-floating-icon" aria-hidden="true">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="container">
        <div className="card" style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ lineHeight: '1.8', fontSize: '18px' }}>
            ...
            {/* Social Media Links - WhatsApp moved to floating button */}
            {(content.instagram_url || content.tiktok_url) && (
